...
function appendTranslationFile(filePath) {
  ...
  const file = filePath.split('/').pop().split('.');
  const [component, language] = [file.shift(), file.shift()];
  ...
  console.log(resultTranslationFilePath);
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      [component]: { ...translationFile },
    })
  );
}

['modules', 'ui', 'app'].forEach(dir =>
